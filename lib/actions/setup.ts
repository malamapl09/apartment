...
export async function checkBuildingsExist() {
  const supabase = await createClient();
  const { data, error } = await supabase.rpc("has_any_buildings");
  if (error) return { exists: true }; // fail closed
  return { exists: !!data };
}

export async function completeSetup(formData: FormData) {
  // Double-check no buildings exist (race condition guard)
  const { exists } = await checkBuildingsExist();
  if (exists) return { error: "Setup has already been completed." };
  ...
  if (profileError) {
    // Cleanup: delete building and user
  ...
  if (signInError) {
    return { error: "Setup completed but sign-in failed. Please log in manually." };
  }
